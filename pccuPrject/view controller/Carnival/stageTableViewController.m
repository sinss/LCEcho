...
- (void)viewDidLoad
{
    ...
    /*
     隔線刪除
     */
    if (downloadStage == nil)
    {
        NSString *tabUrl = [[appConfigRecord appConfigInstance] tabURL2];
        if (tabUrl != nil)
        {
            downloadStage = [[downloadStoreDelegate alloc] initWithURL:tabUrl];
            downloadStage.currLocation = [[appConfigRecord appConfigInstance] currentLocation];
            downloadStage.csvLoadtype = csvLoadTypeStage;
            [downloadStage setDelegate:self];
        }
    }
...
- (void)dealloc
{
    ...
    [downloadStage release], downloadStage = nil;
...
{
    stageRecord *stage = [stageArray objectAtIndex:[indexPath row]];
    ...
    [detailView setDetailType:carnivalDetailTypeStage];
    [detailView setCurrentStage:stage];
...
- (NSURL*)lazyImageURLForIndexPath:(NSIndexPath*)indexPath
{
	...
	return [NSURL URLWithString:stage.sImageUrl];
}
...
{
    stageArray = [[NSArray alloc] initWithArray:stores];
    //NSLog(@"%@",storeArray);
...
- (void)reloadStoreData
{
    [downloadStage startGetStoreWithRefreshing:NO];
}
- (void)reloadStoreDataOfRefreshing
{
    [downloadStage startGetStoreWithRefreshing:YES];
}
